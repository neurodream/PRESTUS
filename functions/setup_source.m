...
for transducer_idx = 1:numel(parameters.transducers)
    ...
    for el_i = 1:transducer_pars.n_elements
        if parameters.n_sim_dims == 3
            bowl = makeBowl(grid_dims, trans_pos_current, transducer_pars.radius_grid, transducer_pars.Elements_OD(el_i), focus_pos_current);
        else
            bowl = makeArc(grid_dims, trans_pos_current, transducer_pars.radius_grid, transducer_pars.Elements_OD(el_i), focus_pos_current);
        end
        if transducer_pars.Elements_ID(el_i) > 0
            if parameters.n_sim_dims == 3
                bowl = bowl - makeBowl(grid_dims, trans_pos_current, transducer_pars.radius_grid, transducer_pars.Elements_ID(el_i), focus_pos_current);
            else
                bowl = bowl - makeArc(grid_dims, trans_pos_current, transducer_pars.radius_grid, transducer_pars.Elements_ID(el_i), focus_pos_current);
            end
        end

        % Define the binary source mask and label the elements
        transducer_mask = transducer_mask + bowl;
        source_labels.transducer = source_labels.transducer + bowl * transducer_idx;
        source_labels.element = source_labels.element + bowl * el_i;
...
p_mask_source_p_el = source_labels.element;
p_mask_source_p_el(p_mask_source_p_el(:,:,:) == 0) = [];
p_mask_source_p_el = reshape(p_mask_source_p_el, [], 1);

p_mask_source_p_tr = source_labels.transducer;
p_mask_source_p_tr(p_mask_source_p_tr(:,:,:) == 0) = [];
p_mask_source_p_tr = reshape(p_mask_source_p_tr, [], 1);

% Initialize source signal
source.p = zeros(length(p_mask_source_p_el), length(cw_signals_all{1}));

for ii = 1:length(p_mask_source_p_el)
    transducer_idx = p_mask_source_p_tr(ii);
    element_idx = p_mask_source_p_el(ii);
...
if parameters.use_kWaveArray ~= 0
    ...
    for transducer_idx = 1:numel(parameters.transducers)
        ...
        focus_pos = parameters.transducers(transducer_idx).focus_pos_final;
        transducer_pars = parameters.transducers(transducer_idx);
    ...
    for transducer_idx = 1:numel(parameters.transducers)
        for el_i = 1:parameters.transducers(transducer_idx).n_elements
            idx = el_i + (transducer_idx - 1) * parameters.transducers(1).n_elements;
