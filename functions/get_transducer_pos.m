...
target_L = [data.x_l(row_idx), data.y_l(row_idx), data.z_l(row_idx)];
target_L = target_L + target_pos_add([1 2 3]);
target_R = [data.x_r(row_idx), data.y_r(row_idx), data.z_r(row_idx)];
target_R = target_R + target_pos_add;
if strcmp(side, 'l')
    target = target_L;
    other = target_R;
...
parameters.transducers(transd_ind).focus_pos_t1_grid = round(target);  % flip back to normal space
