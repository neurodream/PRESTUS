function parameters = load_parameters(varargin)
    ...
    for i = 1:nargin
        ...
        extra_params = yaml.loadFile(fullfile('configs', extra_config_file), "ConvertToArray", true);
        parameters = MergeStruct(parameters, extra_params);
    end
    ...
    if isfield(parameters, 'transducer') && isfield(parameters, 'transducers')
    ...
    for t_i = 1:numel(parameters.transducers)
        ...
        if ~isfield(parameters.transducers(t_i),'name')
            parameters.transducers(t_i).name = 'default_transducer';
        end
