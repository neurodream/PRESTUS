function data = add_sim_result_patch(data_folder, varargin)
    ...
    fileInfo = dir(fullfile('M:\Documents\repos\PRESTUS_forked\data\sims', data_folder, '*final_heating_orig_coord.nii.gz'));
    ...
    if numel(head_data) > 0
        ...
        within_brain = ismember(head_data, [4 7 8]); % skull
    ...
    x_max = 200; y_max = 200;
    data_ = data;
    while x_max >= 200 || y_max >= 200 
        [max_val, linear_index] = max(data(:));           % Find the max value and its linear index
        [x_max, y_max, z_max] = ind2sub(size(data), linear_index); % Convert linear index to subscripts
        
        % Set the current maximum value in 'data' to a very small value so it won't be selected again
        data(linear_index) = -Inf;
        
        % Store the position of the current maximum if needed
    ...
    disp('max Isppa: ');
    disp(['  ' num2str(data_(x_max, y_max, z_max)) ' W/cm^2']);
