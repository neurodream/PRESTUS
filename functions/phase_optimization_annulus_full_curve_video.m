function [error, ax1, ax2, h, i_axial_oneil] = phase_optimization_annulus_full_curve_video(phase_in, parameters, velocity, axial_position, desired_intensity_curve, plot_results, opt_limits, weights, video, fig_handle)
  arguments
      phase_in
      parameters
      velocity
  ...
  % TODO remove video and fig_handle
  ...
  if plot_results
      ...
      ylim([0 1.5*250]); %max(desired_intensity_curve)
      yyaxis right
      ...
      ylim([0 1.5*max(desired_intensity_curve)]);
