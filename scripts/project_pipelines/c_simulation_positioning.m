...
distinguisher = '_75mm_artprofile_shallower'; % label to for output folder to distinguish from other sims with otherwise same parameters

targets = [...
    % 128 142 156; ... % rNBM lower
    % 128 142 160; ... % rNBM higher
    % 115 140 146; ... % rNBM deeper
    % 115 140 152; ... % rNBM deeper higher
    % 115 140 140; ... % rNBM deeper lower
    % 177 148 161; ... % r control
    % 80 136 138; ... % +50 units depth
    % 61 134 133; ... % +70 units depth
    % 61 134 133; ... % +100 units depth
    % 109 140 145; ... % very high transducer angle
    % 50 132 132; ... % inside skin (just as test)
    % 50 132 132; ... % inside skull (just as test)
    % 128 142 150; ... % lower transducer
    % 128 142 150; ... % lower still, angled upwards
    61 134 167; ... % lower still, angled upwards, deeper steering
    % 129 142 155; ...; % subject 2 low & angled upwards
    ];

positions = [...
    % 212 144 172; ... % rNBM lower
    % 209 146 180; ... % rNBM higher
    % 212 152 169; ... % rNBM deeper
    % 212 152 169; ... % rNBM deeper higher
    % 212 152 159; ... % rNBM deeper lower
    % 212 152 170; ... % r control
    % 212 152 170; ... % +50 units depth
    % 212 152 170; ... % +70 units depth
    % 212 152 170; ... % +100 units depth
    % 195 145 219; ... % very high transducer angle
    % 193 150 165; ... % inside skin (just as test)
    % 174 148 161; ... % inside skull (just as test)
    % 212 152 150; ... % lower transducer
    % 212 152 130; ... % lower still, angled upwards
    212 152 130; ... % lower still, angled upwards, deeper steering
    % 211 144 118; ... % subject 2 low & angled upwards
    ];

% transducer_pos = [202 150 200];
% target_pos = [128 142 170];
...
% amp = 72850; % for 30 W/cm^2
% amp = 115608; % for ~70 W/cm^2
% amp = 74038; % linear interpolation between prev. 2 attempts to stay within 2° heating
% amp = 94229; % average from first two values
amp = 105000; % yet another ~average between 94229 (37.8°) and 115608 (95°)
amp = 200000; % yet another ~average between 94229 (37.8°) and 115608 (95°)
% amp = 105001; % yet another ~average between 94229 (37.8°) and 115608 (95°) % one more unit to distinguish from last run - here higher angle
% amp = 105002; % yet another distinguisher for insula control
% amp = 500000; % crank up so to see effects with few trials

duration = trials.*.2; % in s (aka .2 means 200 ms PRF) % TODO!!! necessary?

% unclear how it maps unto config vars
...
% mni_targets = struct('right_MD', [6.5 -18 9], ...
%     'right_amygdala',[26 -4 -20],...
%     'right_PUL',[15 -27 6.5], ...
%     'right_CM', [11 -18 4]);
% mni_targets = struct('BF_right_test1',[107 127 60]);
% all_targets = {'NBM_right', 'NBM_left'};
% all_targets = {'NBM_right'};

% TODO figure out where to put target data; for now hardcoded
% insula_target_data = load('H:\sleep\nicade\Documents\PRESTUS_offline_Martin_NA2\data_higher_angle\subjects_data.mat');
% insula_target_data = load('/home/sleep/nicade/Documents/PRESTUS_offline_Martin_NA2/data_higher_angle/subjects_data.mat');
% insula_target_data = insula_target_data.subjects;

% TODO enable a dry run option
...
for i = 1:size(targets, 1)
    ...
    % pn.data_sims = fullfile(rootpath, 'data_higher_angle', 'tussim', transducer_name);
    ...
    % parameters.bids_path = fullfile(rootpath, '..','amythal_BIDS','data','bids', ['sub-00',num2str(subject_id)],'ses-mri01', 'anat');
    % parameters.t1_path_template = fullfile('..', '..', 'scans', ['sub-00',num2str(subject_id) '*T1*.nii.gz']);
    % parameters.t2_path_template = fullfile('..', '..', 'scans', ['sub-00',num2str(subject_id) '*T2*.nii.gz']);
    ...
    %     '_duty_', num2str(dutycycle*100), ...
    %     '_trials_', num2str(trials), ...
    %     '_amp_', num2str(amp), ...
    %     ];
    
    % single_subject_pipeline(subject_id, opt_parameters);

    %TODO change back to qsub
    single_subject_pipeline_with_qsub(subject_id, opt_parameters, 60*60*12, 64);
    % single_subject_pipeline(subject_id, opt_parameters);
    
    % % create file to check for transfer
    % filename = sprintf('%s.txt', opt_parameters.results_filename_affix);
    % fid = fopen(fullfile(opt_parameters.data_path, 'tussim', transducer_name, sprintf('sub-%03d', subject_id), filename), 'w');
    % fclose(fid)
