...
transducer_profile_name = 'Imasonic_test_ISPPA_80mmmax_sim'; % transducer_name; %'Imasonic_test_ISPPA_50mm';
...
distinguisher = '_MWE_replicate_test1';
...
amps = [160000 180000 200000 202000 204000 206000 208000 210000 212000 214000 216000];
amps = [216000];
...
for i = 1:size(targets, 1)
    ...
    for j = 1:size(amps, 2)
        ...
        % TODO NOTE: replaced with boxcar function of desired focus (TODO
        % write params at the beginning of the script)
        boxcar = real_profile;
        boxcar(:,2) = create_boxcar(50, 10, real_profile(:, 1));
        real_profile = boxcar;
        ...
        h = figure('Position', [10 10 900 500]);
        analytic_intensity = p_axial_oneil .^2/(2*parameters.medium.water.sound_speed*parameters.medium.water.density) .* 1e-4;
        plot(axial_position, analytic_intensity);
        ...
        % weights = ones(size(real_profile, 1)); % TODO careful, changed here
        ...
        param_cmp = [];
        param_cmp.weights = weights;
        param_cmp.opt_phases_and_velocity = opt_phases_and_velocity;
        param_cmp.options = options;
        param_cmp.opt_limits = opt_limits;
        param_cmp.real_profile = real_profile;
        param_cmp.parameters = parameters;
        param_cmp.boxcar = boxcar;
        param_cmp.i_axial_oneil = i_axial_oneil;
        param_cmp.velocity = velocity;


        save('scripts/tests/parameter_comparison_MWE.mat', 'param_cmp');
        ...
        h = figure('Position', [10 10 900 500]);
        orig_intensity_vals = p_axial_oneil.^2/(2*parameters.medium.water.sound_speed*parameters.medium.water.density) .* 1e-4;
        writematrix(orig_intensity_vals, fullfile(outputs_folder, 'simulated_profile.csv'));
        plot(axial_position, orig_intensity_vals);
        xlabel('Axial Position [mm]');
        ylabel('Intensity [W/cm^2]');
        hold on
        intensity_vals = p_axial_oneil_opt .^2/(2*parameters.medium.water.sound_speed*parameters.medium.water.density) .* 1e-4;
        plot(axial_position, intensity_vals);
        writematrix(intensity_vals, fullfile(outputs_folder, 'simulated_profile_opt.csv'));
        plot(real_profile(:,1),real_profile(:,2))
        hold off
        xline(parameters.expected_focal_distance_mm, '--');
        yline(30, '--');
