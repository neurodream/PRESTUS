...
for sbj_ID = sbj_IDs%1:6
    ...
    try
        filename_glob = sprintf('sub-%03d/sub-%03d_layered_heating_resL+z--r_R+z--r_it*_heatingtimeline_imprecisionnone.mat', sbj_ID, sbj_ID);
        f = dir(fullfile(filepath, filename_glob));
        load(fullfile(f.folder, f.name), 'time_status_seq', 'tissue_CEM43', 'tissue_heat');
    catch
        filename_glob = sprintf('sub-%03d/sub-%03d_layered_heating_resL--r_R--r_it*_heatingtimeline_imprecisionnone.mat', sbj_ID, sbj_ID);
        f = dir(fullfile(filepath, filename_glob));
        load(fullfile(f.folder, f.name), 'time_status_seq', 'tissue_CEM43', 'tissue_heat');
    end
